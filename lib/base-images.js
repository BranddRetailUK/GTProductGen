...
export async function syncBaseProductImages({ dryRun = true } = {}) {
  const [{ folder, resources }, state] = await Promise.all([listBaseImageResources(), getServiceState()]);
  const matches = [];

  for (const template of state.templates || []) {
    const viewAssets = buildViewAssets(template, resources);
    if (!viewAssets.length) continue;
    matches.push({
  ...
  if (!dryRun && matches.length) {
    await updateServiceState((draft) => {
      for (const match of matches) {
        const template = draft.templates.find((entry) => String(entry.id) === String(match.templateId));
