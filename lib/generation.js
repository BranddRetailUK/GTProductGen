...
function shouldPublishRunToShopify(run, options) {
  if (typeof options?.publishToShopify === "boolean") return options.publishToShopify;
  return run?.publishToShopify !== false;
}

async function markProductShopifyPublishError(productId, error) {
  if (!productId) return;
  await updateServiceState((draft) => {
    const product = (draft.products || []).find((entry) => String(entry.id) === String(productId));
    if (!product) return;
    product.shopifyPublishError = error instanceof Error ? error.message : String(error);
    product.updatedAt = new Date().toISOString();
  });
}

export async function processGenerationRun(runId, options = {}) {
  const state = await getServiceState();
  const run = state.runs.find((entry) => entry.id === runId);
  if (!run) return null;
  const publishToShopify = shouldPublishRunToShopify(run, options);

  await updateServiceState((draft) => {
    const target = draft.runs.find((entry) => entry.id === runId);
    if (!target) return;
    target.publishToShopify = publishToShopify;
    ...
    try {
      ...
      await updateServiceState((draft) => {
        ...
        if (sameTemplate && existingProduct?.shopify) {
          ...
          existingShopify = structuredClone(existingProduct.shopify);
        ...
        delete product.shopifyPublishError;
        ...
        if (targetItem) {
          ...
          targetItem.shopify = existingShopify
            ? {
                productId: existingShopify.productId,
                adminUrl: existingShopify.adminUrl,
                handle: existingShopify.handle,
                status: existingShopify.status,
                reused: true
              }
            : null;
          targetItem.errorMessage = null;
          targetItem.updatedAt = new Date().toISOString();
        }
      });

      let shopifyResult = null;
      if (publishToShopify && !existingShopify?.productGid) {
        shopifyResult = await publishProductToShopify(product.id);
      }
      ...
      await updateServiceState((draft) => {
        ...
        if (targetItem) {
          ...
          targetItem.productId = product.id;
          targetItem.shopify = shopifyResult?.shopify
            ? {
                productId: shopifyResult.shopify.productId,
                adminUrl: shopifyResult.shopify.adminUrl,
                handle: shopifyResult.shopify.handle,
                status: shopifyResult.shopify.status,
                reused: false
              }
            : targetItem.shopify;
          targetItem.updatedAt = new Date().toISOString();
      ...
      await markProductShopifyPublishError(generatedProductId, error);
