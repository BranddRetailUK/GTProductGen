...
import { loadDesignAssetsSeed, loadShopCollectionsSeed, loadTemplateSnapshot } from "./snapshots.js";
...
async function buildInitialState() {
  const [snapshotProducts, designSeed, shopCollections] = await Promise.all([
    loadTemplateSnapshot(),
    loadDesignAssetsSeed(),
    loadShopCollectionsSeed()
  ]);

  const templates = buildSeedTemplates(snapshotProducts);
  const designs = Array.isArray(designSeed) ? clone(designSeed) : [];
  const collections = Array.isArray(shopCollections) ? clone(shopCollections) : [];
