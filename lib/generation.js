...
export async function processGenerationRun(runId) {
  ...
  for (const item of activeRun.items) {
    try {
      ...
      const duplicate = current.products.find(
        (product) =>
          String(product.templateProductId) === String(template.id) &&
          String(product.designAssetId) === String(design.id)
      );

      if (duplicate && !activeRun.forceRerun) {
        await updateServiceState((draft) => {
          const targetRun = draft.runs.find((entry) => entry.id === runId);
          const targetItem = targetRun?.items.find((entry) => entry.id === item.id);
          if (targetItem) {
            targetItem.status = ITEM_STATUS_SKIPPED;
            targetItem.productId = duplicate.id;
            targetItem.updatedAt = new Date().toISOString();
          }
          if (targetRun) {
      ...
      await updateServiceState((draft) => {
        draft.products = draft.products.filter(
          (entry) =>
            !(String(entry.templateProductId) === String(template.id) && String(entry.designAssetId) === String(design.id))
        );
