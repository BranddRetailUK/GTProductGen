...
import { uploadRenderedImage } from "../cloudinary.js";
...
async function resolveBaseAsset(template, viewId, colourName) {
  ...
  const exactMatch = viewAssets.find(
    (asset) =>
      String(asset?.viewId || "front") === String(viewId || "front") &&
      (!asset?.colourName || String(asset.colourName) === String(colourName || ""))
  );
  const fallbackMatch = exactMatch || viewAssets.find((asset) => String(asset?.viewId || "front") === String(viewId || "front"));
