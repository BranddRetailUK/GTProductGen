...
async function resolveBaseAsset(template, viewId, colourName) {
  ...
  const genericViewMatch = viewAssets.find(
    (asset) => String(asset?.viewId || "front") === String(viewId || "front") && !asset?.colourName
  );
  const fallbackMatch =
    exactColourMatch || genericViewMatch || viewAssets.find((asset) => String(asset?.viewId || "front") === String(viewId || "front"));
