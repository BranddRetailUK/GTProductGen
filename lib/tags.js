import { SHOP_AUDIENCES, SHOP_PRODUCT_TYPES } from "./constants.js";
...
export function matchesShopFilters(product, audience, productType) {
  const tags = Array.isArray(product?.tags) ? product.tags : [];
  if (audience && !hasTag(tags, audience)) return false;
  if (productType && !hasTag(tags, productType)) return false;
  return true;
}

export function buildShopHref(audience, productType) {
  if (audience && productType) {
    return `/shop/${slugSegment(audience)}/${slugSegment(productType)}`;
  }
  if (audience) return `/shop/${slugSegment(audience)}`;
  if (productType) return `/shop/all/${slugSegment(productType)}`;
  return "/shop/mens";
}
...
export function titleFromShopSegment(segment) {
  const normalized = normalizeTagToken(String(segment || "").replace(/-/g, " "));
  return [...SHOP_AUDIENCES, ...SHOP_PRODUCT_TYPES].find(
    (value) => normalizeTagToken(value) === normalized
  );
}
