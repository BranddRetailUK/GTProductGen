...
const TEMPORARY_LINK_CACHE_KEY = Symbol.for("product-gen.dropbox-temporary-link-cache");
...
function getTemporaryLinkCache() {
  if (!globalThis[TEMPORARY_LINK_CACHE_KEY]) {
    globalThis[TEMPORARY_LINK_CACHE_KEY] = new Map();
  }
  return globalThis[TEMPORARY_LINK_CACHE_KEY];
}
...
export async function getDropboxTemporaryLink(path) {
  ...
  const cache = getTemporaryLinkCache();
  const cached = cache.get(targetPath);
  if (cached?.link && cached.expiresAt > Date.now() + 60_000) {
    return cached.link;
  }
...
export async function getDropboxThumbnail(path, { size = "w64h64" } = {}) {
  const accessToken = await refreshDropboxAccessToken();
  const targetPath = clean(path);

  if (!accessToken || !targetPath) return null;
...
export async function scanDropboxFolder() {
  ...
  while (hasMore) {
    ...
    for (const entry of entries) {
      ...
      const previewUrl = await getDropboxTemporaryLink(entry.path_lower || entry.path_display);
      seen.push({
        id:
          entry.id ||
